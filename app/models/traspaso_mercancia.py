...
class TraspasoMercancia(BaseObject):
    ...
    tipo = db.Column(db.Enum(TipoTraspaso), nullable=False)
    observaciones = db.Column(db.Text, nullable=True)
    
    # Foreign Keys
    fkEmpresa = db.Column(db.String(36), db.ForeignKey('empresa.oid'), nullable=False)
    fkSucursalOrigen = db.Column(db.String(36), db.ForeignKey('sucursal.oid'), nullable=False)
    fkSucursalDestino = db.Column(db.String(36), db.ForeignKey('sucursal.oid'), nullable=False)
